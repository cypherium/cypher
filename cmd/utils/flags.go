...
var (
	...
	*/
	...
	//added configurable call timeout for execution of calls
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *eth.Config) {
	...
	if ctx.GlobalIsSet(EnabledTPSFlag.Name) {
		cfg.EnableTPS = ctx.GlobalBool(EnabledTPSFlag.Name)
	}
...
// returns also the ethereum service which is used by the raft service
func RegisterEthService(stack *node.Node, cfg *eth.Config) (ethapi.Backend, *eth.Ethereum) {
	if cfg.SyncMode == downloader.LightSync {
		Fatalf("Failed to register the Ethereum service")
		return nil, nil
		/*??
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "cypher.", tagsMap)
		}

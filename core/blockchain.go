...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64, kbc *KeyBlockChain) (*BlockChain, error) {
	...
	var err error
	bc.hc, err = NewHeaderChain(db, chainConfig, engine, bc.insertStopped, kbc)
	if err != nil {
		return nil, err
	}
...
func (bc *BlockChain) CommitBlockWithState(deleteEmptyObjects bool, state *state.StateDB) error {
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool, verifySign bool) (int, error) {

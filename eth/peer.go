...
func (p *peer) broadcastCandidates(removePeer func(string)) {
	for {
		select {
		case candidate := <-p.queuedCandidates:
			if err := p.SendCandidate(candidate); err != nil {
				log.Error("broadcastCandidates", "error", err)
				removePeer(p.id)
				return
			}
			p.Log().Trace("Propagated Candidate", "number", candidate.KeyCandidate.Number, "hash", candidate.Hash())
...
func (p *peer) SendCandidate(candidate *types.Candidate) error {
	...
	p.Log().Trace("Send candidate to peer", "code", CandidateMsg,
		"peer", p.String(),
...
func (ps *peerSet) Register(p *peer, removePeer func(string)) error {
	...
	go p.broadcastCandidates(removePeer)
	return nil
}

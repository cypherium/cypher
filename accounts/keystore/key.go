...
type Key struct {
	...
	PrivateKey25519 ed25519.PrivateKey
}
...
func storeNewKey(ks keyStore, rand io.Reader, auth string, keyType string) (*Key, accounts.Account, error) {
	var (
		key *Key
		err error
	)
	...
	sScheme := KeyStoreScheme
	if keyType == "ed25519" {
		sScheme = KeyStoreScheme25519
	}
	a := accounts.Account{
		Address: key.Address,
...
func keyFileName(keyAddr common.Address, keyType string) string {
	ts := time.Now().UTC()
	if keyType == "ed25519" {
		return fmt.Sprintf("ED25519--%s--%s", toISO8601(ts), hex.EncodeToString(keyAddr[:]))
	}
